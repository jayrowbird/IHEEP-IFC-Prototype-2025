...
namespace HeepWare.Data.Load
{
    ...
    {    
        internal List<IFCPropertySet> LoadProperties(XmlTextReader reader)
        {
            List<IFCPropertySet> propertySets = new List<IFCPropertySet>();
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        if (reader.Name == "IfcPropertySet")
                        {
                            propertySet = new IFCPropertySet();
                            if (reader.HasAttributes)
                            {
                                propertySetAttributes(reader, propertySet);
                        ...
                        {
                            if (propertySet != null)
                                propertyAttributes(reader, propertySet);
                        }
        ...
        private void propertySetAttributes(XmlTextReader reader, IFCPropertySet pSet)
        {
        ...
        private void propertyAttributes(XmlTextReader reader, IFCPropertySet pSet)
        {
            reader.MoveToFirstAttribute();
            IFCProperty property = new IFCProperty(); ;
            for (int i = 0; i < reader.AttributeCount; i++)
            {
